...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"news_sentiment_v3_{datetime.now().strftime('%Y%m%d')}.log"),
...
from news_sentiment_analyzer_enhanced import EnhancedNewsSentimentAnalyzer


class NewsSentimentUploaderV3:
    """향상된 뉴스 감정 분석 및 Supabase 업로드 클래스"""
    ...
    def __init__(self):
        ...
        # 향상된 감정 분석기 초기화
        self.analyzer = EnhancedNewsSentimentAnalyzer()
        logger.info("향상된 감정 분석기 초기화 완료")
    ...
    def analyze_and_save_news(self, news_file: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            # 감정 분석 수행
            logger.info(f"{len(news_df)}개 뉴스 감정 분석 시작...")
            ...
            output_file = os.path.join(processed_dir, f'analyzed_news_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv')
            ...
            logger.info(f"감정 분석 완료 - 긍정: {summary['sentiment_distribution']['긍정']}, "
                       f"부정: {summary['sentiment_distribution']['부정']}, "
                       f"중립: {summary['sentiment_distribution']['중립']}")
            logger.info(f"평균 감정 점수: {summary['average_sentiment_score']}")
            logger.info(f"시장 신호: {summary['market_signal']}")
            
            # 요약 정보도 저장
            summary_file = os.path.join(processed_dir, f'sentiment_summary_{datetime.now().strftime("%Y%m%d")}.json')
                    ...
                    if not existing.data:
                        ...
                        if result.data:
                            ...
                            sentiment_detail = {
                                ...
                                'neutral_score': float(1 - row.get('positive_score', 0) - row.get('negative_score', 0)),
    ...
    def update_statistics(self, df: pd.DataFrame):
        ...
        try:
            ...
            stats_data = {
                ...
                'strong_positive_count': len(df[df['sentiment_score'] >= 0.3]),
                'strong_negative_count': len(df[df['sentiment_score'] <= -0.3])
            }
...
def main():
    """메인 실행 함수"""
    print("=" * 60)
    print(f"향상된 뉴스 감정 분석 V3 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
    
    uploader = NewsSentimentUploaderV3()
    ...
    if not analyzed_df.empty:
        ...
        if uploader.supabase:
            success = uploader.upload_to_supabase(analyzed_df)
            if success:
                print("\n✅ 모든 작업 완료!")
            else:
                print("\n⚠️ 업로드 중 일부 오류 발생")
